...
func TestMemuRace(t *testing.T) {
	for i := 0; i < 1000; i++ {
		p := &Provider{}
		addr := strconv.Itoa(i)
		go func(p *Provider, addr string) {
...
func testMemConnDeadline(
	...
	lis := serve(
		t, Listen, "memu", t.Name(),
		serverReadDeadline, serverWriteDeadline, read > 0)

	client, err := Dial(lis.Addr().Network(), lis.Addr().String())
	if err != nil {
		t.Fatal(err)
	}
	defer client.Close()
...
func TestMemb(t *testing.T) {
	lis := serve(t, Listen, "memb", t.Name(), 0, 0, true)
	testNetConnParallel(t, lis, Dial)
}

func TestMemu(t *testing.T) {
	lis := serve(t, Listen, "memu", t.Name(), 0, 0, true)
	testNetConnParallel(t, lis, Dial)
}
