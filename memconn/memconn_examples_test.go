...
func Example_http() {
	...
	lis, err := Listen("memu", "MyNamedNetwork")
	if err != nil {
	...
	// Create a new HTTP client that delegates its dialing to
	client := &http.Client{
		Transport: &http.Transport{
			DialContext: func(
				ctx context.Context, _, _ string) (net.Conn, error) {
				return DialContext(ctx, "memu", "MyNamedNetwork")
			},
		},
	}
...
func Example_http_mapped_network() {
	...
	// "DialContext" with the HTTP client transport.
	lis, err := Listen("memu", "host:80")
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
